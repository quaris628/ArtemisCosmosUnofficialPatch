...
class LoopStartRuntimeNode(MastRuntimeNode):
    def enter(self, mast, task:'MastAsyncTask', node:LoopStart):
        ...
        if scoped_cond is None:
            ...
            if node.is_while:
                ...
                _qlog_mast_line(f"eval_code loop start enter {node.file_num} line {node.line_num}: {node.line}")
    ...
    def poll(self, mast, task, node:LoopStart):
        ...
        if node.is_while:
            ...
            if node.code:
                from data.missions.common.q_logger import _qlog_mast_line
                _qlog_mast_line(f"eval_code loop start poll {node.file_num} line {node.line_num}: {node.line}")
                value = task.eval_code(node.code)
                if value == False:
...
class LoopBreakRuntimeNode(MastRuntimeNode):
    ...
    def poll(self, mast, task, node:LoopBreak):
        if node.if_code:
            from data.missions.common.q_logger import _qlog_mast_line
            _qlog_mast_line(f"eval_code loop break {node.file_num} line {node.line_num}: {node.line}")
