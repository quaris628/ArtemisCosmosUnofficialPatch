    ...
    def first_true(self, task: 'MastAsyncTask', node: IfStatements):
        ...
        for i in node.if_chain:
            ...
            if test_node.code:
                ...
                qlog(qlog_level_debug(), f"eval_code conditional if file {node.file_num} line {node.line_num}: {node.line}")
...
class MatchStatementsRuntimeNode(MastRuntimeNode):
    ...
    def first_true(self, task: 'MastAsyncTask', node: MatchStatements):
        ...
        for i in node.chain:
            ...
            if test_node.code:
                from data.missions.common.q_logger import qlog, qlog_level_debug
                qlog(qlog_level_debug(), f"eval_code conditional match file {node.file_num} line {node.line_num}: {node.line}")
                value = task.eval_code(test_node.code)
                if value:
                    cmd_to_run = i
                    break
