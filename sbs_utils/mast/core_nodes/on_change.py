...
class OnChangeRuntimeNode(MastRuntimeNode):
    def enter(self, mast:'Mast', task:'MastAsyncTask', node: OnChange):
        ...
        if not node.is_end:
            ...
            _qlog_mast_line(f"eval_code on_change enter file {node.file_num} line {node.line_num}: {node.line}")
    ...
    def test(self):
        ...
        self.is_running = False
        from data.missions.common.q_logger import _qlog_mast_line
        _qlog_mast_line(f"eval_code on_change test file {self.node.file_num} line {self.node.line_num}: {self.node.line}")
