...
class Button(MastNode):
    ...
    def resolve_data_context(self, task):
        if self.data is not None and not isinstance(self.data, dict):
            ...
            qlog(qlog_level_debug(), f"eval_code button resolve_data_context self.data={self.data} type(self.data)={self.data}")
    ...
    def run(self, task, button_promise):
        ...
        if self.data is not None and not isinstance(self.data, dict):
            from data.missions.common.q_logger import qlog, qlog_level_debug
            qlog(qlog_level_debug(), f"eval_code button run self.data={self.data} type(self.data)={self.data}")
