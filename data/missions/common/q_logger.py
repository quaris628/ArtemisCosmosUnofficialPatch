...
def initialize_qlog():
    ...
    stack_trace_file.write(f"{time()}\nIf the server hard crashes, then a python stack trace might be written to this file. If the currently-running mission script ends normally, then this file should get deleted, unless the entire game server window gets closed.\n")
...
_QLOG_HARD_CRASH_STACK_TRACE_FILENAME_SUFFIX = "hard-server-crash-stack-trace"
...
def _qlog_get_hard_crash_stack_trace_filepath_relative_to_artemis_cosmos_folder():
    cleaned_timestamp = _qlog_get_cleaned_timestamp()
    return f"{_qlog_get_logs_folder_filepath_relative_to_artemis_cosmos_folder()}q-log {cleaned_timestamp} {_QLOG_HARD_CRASH_STACK_TRACE_FILENAME_SUFFIX}.txt"
